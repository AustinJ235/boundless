...
impl WASAPIPlayback {
    pub fn new() -> Result<Box<dyn AudioEndpoint + Send + Sync>, String> {
        ...
        let thrd_h = thread::spawn(move || unsafe {
            ...
            if let Err(e) = audio_client.GetService(
                &IAudioRenderClient::IID,
                mbu_render_client.as_mut_ptr() as *mut _,
            ) {
        ...
        match message {
            Message::AudioChunk {
                ...
                if self.stream_info.0 != channels {
                    println!(
                        "[Audio]: Rejected audio chunk, reason: expected {} channels, but recv'd \
                         {} channels.",
                    ...
                    println!(
                        "[Audio]: Rejected audio chunk, reason: expected sample rate of {} Hz, \
                         but recv'd {} Hz.",
                    ...
                    match self.send_queue.try_send(data) {
                        ...
                        Err(TrySendError::Disconnected(_)) => {
                            Err(String::from("playback thread has exited."))
                        },
                    }
                }
            },
