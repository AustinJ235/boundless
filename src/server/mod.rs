...
impl Server {
    pub fn new(listen_addr: SocketAddr, audio_enable: bool) -> Result<Arc<Worm<Self>>, String> {
        ...
        let audio_endpoint = match audio_enable {
            true => {
                Some(
                    new_audio_endpoint()
                        .map_err(|e| format!("Failed to initialize audio endpoint: {}", e))?,
                )
            },
            false => None,
        };
        ...
        let socket = SecureSocket::listen(
            host_keys,
            listen_addr,
            on_receive,
            on_connect,
        ...
        match message.ty() {
            MessageTy::ClientFeatures => {
                self.send_message(Message::ServerFeatures {
                    audio: self
                        .audio_endpoint
                        .as_ref()
                        .map(|endpoint| endpoint.stream_info()),
                });
            },
            MessageTy::AudioChunk => {
                match self.audio_endpoint.as_ref() {
                        ...
                        match some.send_message(message) {
                            ...
                            Err(e) => {
                                self.signal_error(format!(
                                    "Failed to send audio chunk to endpoint: {}",
                                    e
                                ))
                            },
                        }
                    ...
                    None => {
                        println!(
                            "[Server]: Received audio chunk from client, but audio is not enabled!"
                        )
                    },
                }
            },
    ...
    pub fn wait_for_exit(&self) -> Result<(), String> {
        ...
        while error_gu.is_none() {
            ...
            self.error_cond
                .wait_for(&mut error_gu, Duration::from_secs(1));
        }
