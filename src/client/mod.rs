...
impl Client {
    pub fn new(connect_to: SocketAddr, audio_enable: bool) -> Result<Arc<Worm<Self>>, String> {
        ...
        let input_endpoint = new_input_endpoint()
            .map_err(|e| format!("Failed to initialize input endpoint: {}", e))?;
        ...
        let socket =
            SecureSocket::connect(host_keys, connect_to, on_receive, on_connect, on_disconnect)
    ...
    fn on_disconnect(&self) {
        self.audio_source
            .as_ref()
            .map(|audio_source| audio_source.set_stream_info(None));
    }
                ...
                match message {
                    ...
                    message => {
                        match self.input_endpoint.send_message(message) {
                            Ok(_) => (),
                            Err(e) => {
                                self.signal_error(format!("Failed to send message to input: {}", e))
                            },
                        }
                    },
                }
