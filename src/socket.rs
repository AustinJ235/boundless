...
impl Peer {
    ...
    fn last_hdshk_recv(&self) -> Option<Instant> {
        let mut negotiated_insts: Vec<_> = self
            .crypto
            .values()
            .map(|crypto| &crypto.negotiated)
            .collect();
...
impl Crypto {
    ...
    fn encrypt(
        ...
        let out = self
            .cipher
            .encrypt(&Nonce::from(nonce_bytes), message.as_slice())
            .map_err(|_| SSError::Encrypt)?;
    ...
    fn decrypt(&mut self, seq: u64, encrypted: &[u8], length: bool) -> Result<Vec<u8>, SSError> {
        ...
        if length {
            ...
            let length =
                u16::from_le_bytes(<[u8; 2]>::try_from(&decrypted[0..2]).unwrap()) as usize;
...
impl SecureSocket {
    ...
    pub fn connect(
        ...
        let udp_socket =
            Arc::new(UdpSocket::bind("0.0.0.0:0").map_err(|e| SSError::Bind(e.kind()))?);
        udp_socket
            .set_write_timeout(Some(Duration::from_millis(350)))
            .unwrap();
        udp_socket
                        ...
                        Mode::Server => {
                            ...
                            socket.peer.lock().retain(|peer_id, peer| {
                                if peer.last_ping_recv().elapsed() > PING_DISCONNECT {
                                    println!(
                                        "[Socket]: Connection lost to {}, reason: no ping \
                                         received.",
                                        peer.addr
                                    );
                                    ...
                                    match peer.last_hdshk_recv() {
                                        Some(inst) => {
                                            if inst.elapsed() > HANDSHAKE_DISCONNECT {
                                                println!(
                                                    "[Socket]: Connection lost to {}, reason: no \
                        ...
                        Mode::Client => {
                            ...
                            if peer.crypto.is_empty() {
                                if !peer.hdshk_pending()
                                    || peer.last_hdshk_sent().elapsed() >= HANDSHAKE_TIMEOUT
                                {
                                    ...
                                    let mut send_buf =
                                        Vec::with_capacity(98 + signature_bytes.len());
                                    ...
                                    if let Err(e) = udp_socket.send_to(&*send_buf, peer.addr) {
                                        println!(
                                            "[Socket]: Connection lost to {}, reason: failed to \
                                             send packet: {}.",
                                            peer.addr,
                                            e.kind()
                                        );
                                ...
                                if peer.last_ping_sent().elapsed() > PING_INTERVAL {
                                    ...
                                    if let Err(e) = socket.send_internal(
                                        ...
                                        println!(
                                            "[Socket]: Connection lost to {}, reason: {}.",
                                            peer.addr, e
                                        );
                                        peer.crypto.clear();
                                ...
                                if peer.last_hdshk_recv().unwrap().elapsed() > HANDSHAKE_INTERVAL {
                                    if peer.hdshk_pending() {
                                        ...
                                        let mut signature_bytes =
                                            socket.keys.sign_message(&*signed_message);
                                        let mut message =
                                            Vec::with_capacity(signature_bytes.len() + 65);
                                        ...
                                        peer.set_hdshk_sent();
                                        let addr = peer.addr.clone();
                                        let (old_hdshk_id, old_crypto) =
                                            peer.latest_crypto().unwrap();
                                        println!(
                                            "[Socket]: Renegotiating connection with {}.",
                                            addr
                                        );
                ...
                match udp_socket.recv_from(&mut *recv_buf) {
                    Ok((len, addr)) => {
                        ...
                        match PacketType::from_byte(&recv_buf[0]) {
                            Some(PacketType::HdshkReq) => {
                                if mode != Mode::Server {
                                    println!(
                                        "[Socket]: Rejected HdshkReq from {}, reason: unexpected \
                                         packet type.",
                                        addr
                                    );
                                    continue;
                                ...
                                if len < 34 {
                                    println!(
                                        "[Socket]: Rejected HdshkReq from {}, reason: packet is \
                                         truncated (c1).",
                                        addr
                                    );
                                    continue;
                                ...
                                if let Err(e) = socket.keys.verify_message(
                                    peer_id,
                                    signature_bytes,
                                    message_bytes,
                                ) {
                                    println!(
                                        "[Socket]: Rejected HdshkReq from {}, reason: \
                                         verification failed: {}",
                                        addr, e
                                    );
                                    continue;
                                ...
                                let peer_public = match PublicKey::from_sec1_bytes(
                                    &message_bytes[17..public_end],
                                ) {
                                    Ok(ok) => ok,
                                    Err(_) => {
                                        println!(
                                            "[Socket]: Rejected HdshkReq from {}, reason: invalid \
                                             public key.",
                                            addr
                                        );
                                        continue;
                                ...
                                let public = EncodedPoint::from(private.public_key());
                                let secret = hash(
                                    private.diffie_hellman(&peer_public).as_bytes().as_slice(),
                                );
                                ...
                                let mut signature_bytes =
                                    socket.keys.sign_message(&*signed_message);
                                let mut send_buf = Vec::with_capacity(
                                    34 + signature_bytes.len() + signed_message.len(),
                                );
                                ...
                                if let Err(e) = udp_socket.send_to(&*send_buf, &addr) {
                                    println!(
                                        "[Socket]: Failed to send HdshkReq to {}, reason: failed \
                                         to send packet: {}",
                                        addr,
                                        e.kind()
                                    );
                            ...
                            Some(PacketType::HdshkRes) => {
                                if mode != Mode::Client {
                                    println!(
                                        "[Socket]: Rejected HdshkRes from {}, reason: unexpected \
                                         packet type.",
                                        addr
                                    );
                                    continue;
                                ...
                                if len < 34 {
                                    println!(
                                        "[Socket]: Rejected HdshkRes from {}, reason: packet is \
                                         truncated (c1).",
                                        addr
                                    );
                                    continue;
                                ...
                                if let Err(e) = socket.keys.verify_message(
                                    ...
                                    println!(
                                        "[Socket]: Rejected HdshkRes from {}, reason: \
                                         verification failed: {}",
                                        addr, e
                                    );
                                    continue;
                                ...
                                let peer_public = match PublicKey::from_sec1_bytes(
                                    ...
                                    Err(_) => {
                                        println!(
                                            "[Socket]: Rejected HdshkRes from {}, reason: invalid \
                                             public key.",
                                            addr
                                        );
                                        continue;
                                ...
                                if addr != peer.addr {
                                    println!(
                                        "[Socket]: Rejected HdshkRes from {}, reason: address \
                                         mismatch.",
                                        addr
                                    );
                                    continue;
                                ...
                                let decrypted = match crypto.decrypt(
                                    ...
                                    Err(_) => {
                                        println!(
                                            "[Socket]: Rejected HdshkRes from {}, reason: test \
                                             failed (c1).",
                                            addr
                                        );
                                        continue;
                                ...
                                if decrypted.len() != 64 {
                                    println!(
                                        "[Socket]: Rejected HdshkRes from {}, reason: test failed \
                                         (c2).",
                                        addr
                                    );
                                    continue;
                                ...
                                if hash(&decrypted[32..])
                                    != <[u8; 32]>::try_from(&decrypted[0..32]).unwrap()
                                {
                                    println!(
                                        "[Socket]: Rejected HdshkRes from {}, reason: test failed \
                                         (c3).",
                            ...
                            Some(packet_ty) => {
                                match packet_ty {
                                    PacketType::EncHdshkRes => {
                                        if mode != Mode::Client {
                                            println!(
                                                "[Socket]: Rejected EncHdshkRes from {}, reason: \
                                    ...
                                    PacketType::EncHdshkReq => {
                                        if mode != Mode::Server {
                                            println!(
                                                "[Socket]: Rejected EncHdshkReq from {}, reason: \
                                                 unexpected packet type.",
                                                addr
                                            );
                                            continue;
                                ...
                                if len < 137 {
                                    println!(
                                        "[Socket]: Rejected {} from {}, reason: packet is \
                                         trucated (c1).",
                                        packet_ty, addr
                                    );
                                    continue;
                                ...
                                let hdshk_id: HandshakeID =
                                    <[u8; 16]>::try_from(&recv_buf[33..49]).unwrap();
                                let seq = u64::from_le_bytes(
                                    <[u8; 8]>::try_from(&recv_buf[49..57]).unwrap(),
                                );
                                let mut peers = socket.peer.lock();
                                ...
                                let peer = match match mode {
                                    ...
                                    None => {
                                        println!(
                                            "[Socket]: Rejected {} from {}, reason: no active \
                                             conection.",
                                            packet_ty, addr
                                        );
                                        continue;
                                ...
                                let crypto = match peer.crypto.get_mut(&hdshk_id) {
                                    ...
                                    None => {
                                        println!(
                                            "[Socket]: Rejected {} from {}, reason: invalid \
                                             handshake.",
                                            packet_ty, addr
                                        );
                                        continue;
                                ...
                                let message = match crypto.decrypt(
                                    ...
                                    Err(_) => {
                                        println!(
                                            "[Socket]: Rejected {} from {}, reason: decryption \
                                             failed.",
                                            packet_ty, addr
                                        );
                                        continue;
                                ...
                                match packet_ty {
                                    PacketType::EncHdshkReq => {
                                        ...
                                        let signed_message =
                                            &message[signature_end..(signature_end + 64)];
                                        ...
                                        let mut res_signed_msg =
                                            Vec::with_capacity(public.as_bytes().len() + 82);
                                        ...
                                        let mut signature_bytes =
                                            socket.keys.sign_message(&*res_signed_msg);
                                        let mut res_message = Vec::with_capacity(
                                            signature_bytes.len() + res_signed_msg.len() + 1,
                                        );
                                        debug_assert!(signature_bytes.len() + 1 <= 255);
                                        ...
                                        println!(
                                            "[Socket]: Renegotiated connection with: {}.",
                                            addr
                                        );
                                    },
                                    PacketType::EncHdshkRes => {
                                        ...
                                        if message.len()
                                            != message[signature_end + 16] as usize
                                                + signature_end
                                                + 80
                                        {
                                            println!(
                                        ...
                                        let test_msg =
                                            match new_crypto.decrypt(0, enc_test_msg, false) {
                                                Ok(ok) => ok,
                                                Err(_) => {
                                                    println!(
                                                        "[Socket]: Rejected {} from {}, reason: \
                                        ...
                                        if hash(&test_msg[32..])
                                            != <[u8; 32]>::try_from(&test_msg[0..32]).unwrap()
                                        {
                                            println!(
                                                "[Socket]: Rejected {} from {}, reason: test \
                                                 failed (c2).",
                                                packet_ty, addr
                                            );
                                            continue;
                                    ...
                                    PacketType::Ping => {
                                        if hash(&message[32..64])
                                            != <[u8; 32]>::try_from(&message[0..32]).unwrap()
                                        {
                                            println!(
                                                "[Socket]: Rejected {} from {}, reason: test \
                                                 failed.",
                                        ...
                                        match mode {
                                            ...
                                            Mode::Client => {
                                                println!(
                                                    ...
                                                    peer.last_ping_sent().elapsed().as_micros()
                                                        as f32
                                                        / 1000.0
                                                );
                                                peer.set_ping_recv();
                            ...
                            None => {
                                println!(
                                    "[Socket]: Rejected packet from {}, reason: invalid packet \
                                     type.",
                                    addr
                                )
                            },
                    ...
                    Err(e) => {
                        match e.kind() {
                            ...
                            _ => {
                                println!(
                                    "[Socket]: Failed to receive packet, reason: {}.",
                                    e.kind()
                                )
                            },
                        }
